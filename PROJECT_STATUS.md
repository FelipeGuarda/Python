...
**Second pass: the ingest was audited procedurally, and it reconciles.** Everything executed:
the published contract is true of the parquets and `deployments.csv` on disk right now
(`diff(load(), build())` → **0 entries**); the registry matches its owning CSV at 27 stations
and agrees with the contract on every label; all 36 figures and `overlap_stats.csv` are newer
than the data they came from, so no output is stale; and every deployment in all three
campaigns carries an explanatory `media_status` — **zero `unexplained`, zero
`no_field_dates`**. Nothing is silently missing.

Two guard-rail gaps were closed on the consumer's side. The two GeoJSONs — the station
registry and the platform's boundary — were read by a bare `st_read`, so a missing one exited
**1** (a crash) instead of **2** (a verdict); both are now checked right after the contract and
refuse with the command that regenerates them. And a station present in the canonical table but
absent from the registry used to *warn*, null the station and exit 0, which was the one path
where records left the analysis without a verdict; it now refuses — proved on a doctored
registry with CT27 removed (93 records, exit 2). No number moved: the overlap table
regenerated byte-identical.

**One item is the producer's.** `estaciones.geojson` is **not covered by the contract** —
`CANONICAL_STATE.json` hashes `deployments.csv` and describes the parquet, but the registry has
no entry, so every coordinate and `altitude_m` crosses unverified. camera-traps guards it
against drift from `estaciones.csv`, but that guarantee is invisible downstream. The fix is a
`stations_sha256` in the published state — a `schema_version` bump to 5 on both sides, so it
is logged in `V2-REVIEW.md` §0-septies rather than made from a consumer's session. It will
matter numerically as soon as the altitude-covariate occupancy is built.
