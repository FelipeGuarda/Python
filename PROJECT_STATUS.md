...
**🔄 IN PROGRESS — camera-traps `docs/V2-REVIEW.md`** (opened 2026-08-18, entry condition met and first items closed 2026-08-19). Felipe's call: **nothing new starts until the camera-trap chain is clean end to end.** Closed 2026-08-19: **1.1** the campaign set is exactly three (pv dropped from `CAMPAIGN_ORDER` and `REPORT_CAMPAIGNS`, kept as provenance), **1.3** the export gate passes for all three from the repo, **1.3b** the reviewer's verdict reaches `observation_type`. Still open: **1.2** the canonical file set (the Timelapse `.ddb`/`.tdb` and the three `addaxai-*` files are undecided), **1.4** manifest coverage — and note primavera's canonical table holds 22 stations not 26, because it is built from the animal-only reviewed CSV so four zero-detection stations contribute no rows at all, which is fine for a numerator and wrong for a trap-effort denominator, **1.5**–**1.9**, **1.10** test fixtures, **1.11** figures not yet re-rendered, **1.12** the two deferred label decisions (21 rows `unknown_pending_taxon`, 3 `unknown_pending_review`, both queryable via the new `review_resolution` column), and all of **§2**, the DuckDB rebuild, which has not started. **Integration Status:** `In Progress [REMAINING: V2-REVIEW 1.2, 1.4–1.12, all of §2]`. **Blockers/Notes:** the DuckDB `ct_*` rebuild is still untouched, so a re-ingest is NOT done until it lands; `timestamps.py` aborts before writing anything on a cp1252 console, so run ingests with `PYTHONIOENCODING=utf-8`; pytest is absent from the `camera-traps` env, use `python -m unittest discover -s tests`.
