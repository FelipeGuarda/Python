...
**Last updated:** 2026-08-18 (camera-traps: **otoño 2026's capture order was never lost, and 103 GB of Synology re-downloads are reclaimed.** Renaming the download folders locally made all three Synology sync tasks treat the originals as missing and restore them beside the flattened trees — otoño 2026 had been carrying a restored copy silently since June. **Safety established before anything was touched:** all three tasks are `sync_direction: 2`, which the daemon log states in words — `is one-way downloading, ignore event` — so nothing local has ever reached the NAS and no local deletion can propagate to it. **The recovery:** `flatten_log_20260616_100329.csv` survived the June flatten, because flattening consumes the tree but not the *record* of the tree. `data/campaigns/otono_2026/dcim_manifest.csv` (5,748 rows, 5,724 moved + 24 renamed) was rebuilt from it by a one-off script that imports `DCIM_MANIFEST_COLUMNS`, `dcim_folder_key` and `stations.canonical_id` rather than restating any of them, then verified three independent ways: per-`(deployment, DCIM folder)` counts against the restored on-disk tree, against the NAS listing in the sync client's `event-db.sqlite`, and against `ImageData_total.csv` (0 manifest rows unjoined; coverage **total** within every described deployment, so `establish_order` cannot refuse them for partial description — the failure mode it warns about). **Result: CT14 goes from 1,633 colliding counters to fully ordered, CT20 from 837, CT23 from 89** — 3,561 frames — and all four described stations earn `ORDER_MANIFEST` where they previously had only `counter`. **A second, unlooked-for result: conservation is now provable.** The NAS holds exactly 2,632 / 1,836 / 1,088 / 192 files for CT14/20/23/24 and the flattened trees hold exactly the same, so the old duplicate-skip discarded **nothing** — closing the CT_14 collision worry open since 2026-07-31, by arithmetic rather than by assumption. **The limit is honest and stated:** CT15 (1,331) and CT08 (1,129) were flattened *before* upload, no folder evidence exists anywhere, and their counters still wrap undetectably. **pv/primavera needed no recovery at all** — its 13,814 remote DCIM files match `primavera_2025/dcim_manifest.csv` exactly, and the 2,460 non-DCIM depth-3 files are the `TC23_M20.2` station un-nested on 2026-08-13; the merged primavera/pv folder raised no discrepancy. **Deletion was gated, not assumed:** every restored file had to have a flattened counterpart matched by *size* (names differ where the flatten renamed on collision) — 10,808 files, **0 unaccounted**, re-checked per station immediately before removal, with skip-not-delete on any mismatch. Freed 103.01 GB; free space 188 → 291 GB. **152 tests still pass.** ⚠️ **The NAS is not a complete backup:** five full-size (~6 MB) CT04 frames in otoño 2025 exist only on the Windows box and are absent from the server — they are *not* the known 0-byte corrupt files. ⚠️ **Structural, unresolved:** a one-way-download sync pointed at a folder we restructure will re-download forever; the tasks are paused, not fixed. Either remove them and keep the trees as working copies, or repoint sync at a pristine mirror and flatten into a separate directory. **Also corrected:** `timestamps.load_manifest`'s docstring claimed otoño 2026 "can never have one". Pending commit + push.)

**⏳ PENDING — camera-traps `docs/V2-REVIEW.md` (opened 2026-08-18, not started).** Felipe's call: **nothing new starts until the camera-trap chain is clean end to end** — canonical files, scripts and outputs, every campaign included — because problems here bleed into the annual report, pehuen and the platform, and arrive there disguised as findings. **Entry condition:** primavera_2025's re-review finished (19,522 rows are `unclassified` today) and its `ImageData_total.csv` passing the export gate. **Camera-traps scope:** three campaigns with pv_2025_2026 retired to provenance; an identical canonical file set per campaign (they differ today); manifest coverage stated per campaign including the stations that legitimately have none; anchors complete or explicitly refused; **one owner for the station registry** — `stations.yaml` (26 stations, **no CT27**) vs the geojson (27) vs `estaciones.csv` (27), which is why CT27's 344 otoño 2026 files ingest coordinateless, and is the CT26 failure repeating; `field_notes.csv` audited beyond the coordinate column; outputs diffed against `figures_pre_reingest/` with every moved number attributed. **Data-pipeline scope:** the DuckDB decision split by regenerability — `ct_*` rebuilt from `observations.parquet` and never migrated, while `weather_station`/`weather_forecast`/`literature` are **not** regenerable and must be recovered from the Linux box and exported to Parquet once, which is what stops the Windows↔Linux migration being a blocker; `timelapse_reviewed.py`'s duplicate derivations retired; `dedup_primavera_2025.py` deleted (its premise dissolves, and the `100EK113` records it excluded become included). **The gate:** camera-traps publishes `CANONICAL_STATE.json`, data-pipeline verifies against a `ct_ingest_state` table and **refuses to report success while they diverge** — fail-closed, living in the tool rather than in memory. Neither piece is written; each gets its own design gate against real re-ingested data. **Boundaries audited:** no import cycle — camera-traps↔data-pipeline share only `species.yaml` with an env override, and the duplicated `species.py` is deliberate and correct; the real defect is Research reaching in by hardcoded absolute Windows paths, which is why pehuen cannot run on Linux. **Out of scope:** the DST instant+offset schema change and the sun-anchored sensitivity run.
