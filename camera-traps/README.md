...
**Integration Status:** In Progress [REMAINING: `timestamps.py` still calls its own `classify_epochs` and applies one offset per station — it does not consume `camtrap/clocks.py` yet, so the segment-aware verdicts above are not yet reflected in `new_labeled_data_corrected.csv` or `observations.parquet`. Also pending: `valid_effort` in `CANONICAL_COLUMNS`, optional `segment_index` in the anchor CSV, the full-category export gate, and the anchor-candidate report. Plan and rationale in `docs/HANDOFF-clock-repair.md` §9.2 steps 2–5.]
