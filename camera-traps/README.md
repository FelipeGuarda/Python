...
**🔄 IN PROGRESS — `docs/V2-REVIEW.md`** (opened 2026-08-18; entry condition met and 1.1 / 1.3 / 1.3b closed 2026-08-19): a comprehensive review that must complete **before any new project starts** — canonical files, scripts and outputs clean across *every* campaign, because problems in this chain do not stay in it. Entry condition: primavera_2025's re-review finished and its export passing the gate. Covers the campaign set (three; pv retired), one owner for the station registry (three registries disagree today — `stations.yaml` has 26 and is missing CT27, which is why CT27's 344 otoño 2026 files ingest with no coordinates), the DuckDB rebuild from the canonical parquet, the stale-code inventory, and a **fail-closed gate so the DuckDB step cannot be silently skipped** (camera-traps publishes `CANONICAL_STATE.json`, data-pipeline verifies and refuses to report success while they diverge). Explicitly out of scope: the DST instant+offset schema change and the sun-anchored sensitivity run.
...
`ImageData_total.csv` holds stills only. Filter the videos out **in Timelapse2 before
exporting**, not out of the CSV afterwards: the CSV is a derived file, so a filter
applied there leaves `TimelapseData.ddb` still holding the videos and the next export
puts them back.

> **Mechanism — TO BE FILLED IN.** Felipe applied this in Timelapse2 for primavera
> 2025; the exact menu path is not recorded here yet. Write it down the next time you
> do it, because the whole point of this rule is that it must survive being forgotten.
