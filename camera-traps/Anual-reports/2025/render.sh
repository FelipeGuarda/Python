#!/usr/bin/env bash
# Render the annual report to DOCX (and optionally PDF).
#
# Usage:
...
# - For PDF output pandoc needs a LaTeX engine (`sudo apt install texlive-xetex`).
...
if ! command -v pandoc >/dev/null 2>&1; then
    echo "pandoc not found. Install it once with: sudo apt install pandoc" >&2
    exit 1
fi
