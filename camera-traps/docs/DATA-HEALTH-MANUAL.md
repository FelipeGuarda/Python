...
> ### ✅ Re-audited 2026-08-24 — A1, A2, A4, B1 and B7 are closed
>
> **Everything on the consumer side is done.** What remains below is the *field-record*
> boundary (A3, B4, B6), pehuén's Windows-side rendering (B2, B3) and Tier C. That split is
> not accidental: it is the finding this part opened with, and taking one boundary as a
> single session's work cleared all of it.
>
> - **A1** — `data/campaigns/estaciones.csv` owns station identity. `stations.yaml` and
>   `camera_trap_stations.geojson` are generated by `setup/build_station_registry.py`, and
>   `tests/test_station_registry.py` fails if either is hand-edited. §1.3's rule now has its
>   test. Stations are `CT01`..`CT27` everywhere; the platform's `TC-01` spelling is gone.
>   **Correction to the text below: CT27 has 315 images, not 344.**
> - **A2** — the contract is verified at the consumer end: `data-pipeline/src/canonical_gate.py`,
>   `python run_fetch.py --ct-check`. It fingerprints the whole campaign description rather
>   than row counts alone, because the 815-row review repair moved verdicts without moving
>   `n_rows`.
> - **A4** — the occupancy denominator is the deployed grid, read from `ct_deployments`.
> - **B1** — `ct_*` rebuilt: 35,807 rows, 74 deployments, reconciling exactly.
> - **B7** — CT27's install is dated 2025-12-11 and its clock is cleared.
>
> **One new item, and it is the same pattern one layer down:** `data-pipeline` now owns four
> modules that guard this boundary, and **nothing guards them** — that project has no test
